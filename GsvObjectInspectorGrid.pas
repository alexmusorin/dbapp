...
type
  ...
  TGsvObjectInspectorInplaceEditor = class(TCustomEdit)
  public
    constructor Create(AOwner: TComponent); override;
  ...
  private
    ...
    function  GetBoolean(Info: PGsvObjectInspectorPropertyInfo): Boolean;
    procedure ChangeBoolean(Info: PGsvObjectInspectorPropertyInfo);
    procedure ExpandingOrChangeBoolean(ExpandingOnly: Boolean = False);
...
begin
  ...
  for i := 0 to Pred(FCount) do
    if FItems[i].HasChildren and FItems[i].Expanded then
      lst.Add(i);
  lst.TopRow   := FInspector.FTopRow;
  lst.Selected := FInspector.FSelected;
  FCount       := 0;
end;
...
begin
  ...
  if FCount = 0 then begin
    ...
    if FHistory.Find(Info^.TheObject.ClassName, i) then
...
begin
  ...
  FInspector         := AOwner as TGsvObjectInspectorGrid;
  Parent             := FInspector;
  ParentColor        := True;
  ParentCtl3D        := True;
...
procedure TGsvObjectInspectorHintWindow.MouseDown(Button: TMouseButton;
...
begin
  ...
  // в области таскбара
  Params.ExStyle := WS_EX_NOPARENTNOTIFY or WS_EX_TOOLWINDOW;
...
procedure TGsvObjectInspectorInplaceEditor.SetNewEditText(
  const Value: String);
begin
  Modified := False;
  if Value = Text then
...
begin
  ...
  if FDropDownCount < 4 then
    FDropDownCount := 4;
  if FListBox.Count < FDropDownCount then
    FDropDownCount := FListBox.Count;
  // корректируем позицию списка
  RDesk := Screen.WorkAreaRect;
  ...
  mw := Width;
  for i := 0 to Pred(FListBox.Count) do
...
begin
  Assert(Assigned(FListBox));
  if Index > FListBox.Count then
    Index := FListBox.Count - 1;
  if Index < 0 then
    Index := 0;
  if Index > FListBox.Count then
    Exit;
  ...
  if Assigned(FListBox) then begin
    if FListBox.Visible then begin
      ...
      case Key of
        ...
        VK_END:    SetListItem(FListBox.Count - 1);
...
begin
  // если видимо окно редактора, то передаем ему фокус, иначе передаем
  // фокус окну инспектора
...
begin
  HideLongHint;
  if (FRowsCount = 0) or (ARow < 0) then
    Exit;
...
begin
  ...
  if sr = FSelected then
    ExpandingOrChangeBoolean(X < FDividerPosition);
end;

// Получение логического значения свойства типа Boolean или элемента
// множества
function TGsvCustomObjectInspectorGrid.GetBoolean(
  Info: PGsvObjectInspectorPropertyInfo): Boolean;
begin
  Assert(Assigned(Info));
  Result := GsvGetBit(DoGetIntegerValue(Info), Info^.Tag);
end;
...
begin
  ...
  // меняем значение признака Extended на противоположное, перестраиваем
...
var
  ...
  begin
    ...
    if GetBoolean(FRows[i]) then sr.Left := FGlyphs.Height * 3
    else                         sr.Left := FGlyphs.Height * 2;
    sr.Top    := 0;
  ...
  begin
    ...
    case FRows[i]^.Kind of
      pkColor,
      pkColorRGB: Canvas.Brush.Color := DoGetIntegerValue(FRows[i]);
    end;
    Canvas.Rectangle(dr);
    Canvas.Pen.Color   := pc;
...
begin
  ...
  for i := FTopRow to iMax do begin
    ...
    if FRows[i]^.Kind = pkFolder then begin
      ...
      r.Right := ClientWidth;
      Canvas.TextRect(r, r.Left, r.Top + dy, FRows[i]^.Caption);
      ...
      DrawLine(clBtnShadow, x1, y0, x1, y1);
      DrawLine(clBtnHighlight, x1 + 1, y0, x1 + 1, y1);
    end;
    ...
    if sel <> FSelected then begin
      ...
      if (FSelected >= 0) and (FSelected < FRowsCount) then begin
        ...
        if (X >= xt) and (X < (xt + FLevelIndent)) then
          ExpandingOrChangeBoolean(True)
        else if ((X > FDividerPosition) and (X < xc)) then
          ExpandingOrChangeBoolean(False);
      end;
    end;
  end;
end;
  ...
  if (Key = #13) or (Key = #32) then begin
    if (FSelected >= 0) and (FSelected < FRowsCount) then begin
      if FRows[FSelected]^.HasChildren then
        ExpandingOrChangeBoolean
      else if HAS(_DIALOG, FRows[FSelected]) then
...
begin
  ...
  if not Assigned(Info) then
    Exit;
  if Assigned(FOnSetStringValue) then
...
begin
  ...
  if (FRowsCount = 0) or (FSelected < 0) or (FSelected >= FRowsCount) then
    Exit;
  if FEditor.Visible and FEditor.Modified then
    ValueChanged(FRows[FSelected], FEditor.Text);
end;
